...
const PreviousExams = () => {
  ...
  const previousExamFlag = useSelector(
    (state) => state.previousExamFlag
  ).previousExamFlag;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const response = await axios.get(`${host}/api/exam/get-all`, {
          headers: {
  ...
  const fetchFloorStatus = async (exam) => {
    let promises = exam.examLocations.map(async (location) => {
      ...
      return data && data.length === location.rooms.length
  ...
  return (
    <div className="text-[14px] w-[100%] overflow-hidden">
      <div className="m-3">
        <h1 className="text-xl sm:text-2xl uppercase font-semibold text-center sm:my-7 mb-7  sm:text-left ">
        ...
        <div id="upcoming-exams" className="my-7">
          ...
          {examsArr?.length != 0 ? (
            <div
              id="exams-cards"
              className="flex min-w-[90vw] overflow-x-auto my-3 gap-2"
            >
              {examsArr?.map((exam, index) => {
                if (auth["user-credentials"].user.userType === "ADMIN") {
                  return (
                    <ExamCard
                      key={`upcoming-exam-${index}`}
                      exam={exam}
                      bgColor={"bg-blue-500"}
                      examType={"UPCOMING"}
                    />
