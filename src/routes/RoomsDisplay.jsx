...
const RoomsDisplay = () => {
  ...
  const { rooms } = exam.examLocations.find(
    (ele) => ele.floorNumber == floorNumber
  );
  console.log(rooms);
  ...
  useEffect(() => {
    ...
    if (
      ...
      thisExamUserArr.length > 0
    ) {
      console.log(thisExamUserArr);
      let user = thisExamUserArr.find(
        (ele) => ele.email == auth["user-credentials"].user.email
      );
      console.log(user);
      const tmp = rooms.find(
        (ele) => ele.roomNumber == user.roomNumber.toString()
      );
      setUserRoom(tmp);
    }
  ...
  const getSortedRooms = (rooms) => {
    const sortedRooms = rooms.slice().sort((a, b) => {
      ...
      if (isNumericA && isNumericB) {
        ...
        return roomNumberA.localeCompare(roomNumberB, undefined, {
          numeric: true,
        });
      }
    });
  ...
  const fetchRoomData = async () => {
    const promises = rooms.map((room) =>
      isRoomAttendanceMarked(room.roomNumber)
    );
  ...
  const handleRoomClick = ({ roomNumber, seatsArr }) => {
    ...
    navigate(`/dashboard/${exam._id}/floor-${floorNumber}/room-${roomNumber}`, {
      replace: true,
    });
  };
    ...
    <div className="my-3 ">
      ...
      <div className="flex justify-center gap-3 floor-room-container flex-wrap overflow-auto">
        {(auth["user-credentials"].user.userType === "ADMIN" ||
          auth["user-credentials"].user.userType === "EXAM_OC") &&
          getSortedRooms(rooms).map((room, index) => (
            <div
              ...
              className={`flex items-center gap-5 cursor-pointer border border-slate-400 p-3 rounded-md hover:shadow-lg ${
                roomsDone.includes(room.roomNumber)
                  ? "bg-green-400 font-medium"
                  : ""
              }`}
            >
        ...
        {userRoom &&
          auth["user-credentials"].user.userType === "INVIGILATOR" && (
            <div
              onClick={() => {
                handleRoomClick(userRoom);
              }}
              className={`flex items-center gap-5 cursor-pointer border border-slate-400 p-3 rounded-md hover:shadow-lg ${
                roomsDone.includes(userRoom.roomNumber)
                  ? "bg-green-400 font-medium"
                  : ""
              }`}
            >
